...
| **10 pose categories** | Standing, Seated, Leaning-Standing, Leaning-Seated, Kneeling, Reclining, Dynamic, Eccentric, Couple, Accessible |
| **12 poses (MVP)** | S-Curve Stand, Soft Sit, Wall Lean, Power Recline, Kneeling Arch, Editorial Arm Reach, etc. |
...
```
poseart-app/
├── index.html              # All 23 screens (SPA, screen visibility toggling)
├── css/
│   ├── tokens.css          # Design tokens (colors, type, spacing, radius, shadow, animation)
│   └── app.css             # Full Art Nouveau styles — all components, screens, animations
└── js/
    ├── poses-data.js       # Pose library (10 categories, 12 poses), in-memory session/favorites storage
