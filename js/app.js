...
window.renderMarketplace = function() {
  ...
  if (_marketplaceSearch) {
    packs = packs.filter(p =>
      ...
      p.description.toLowerCase().includes(_marketplaceSearch)
    );
  }
  ...
  grid.innerHTML = packs.map(p => {
    ...
    const actionBtn = isOwned
      ? `<button class="btn btn-outline" onclick="openPack('${p.id}')" style="width:100%;padding:8px;font-size:12px;">Open Pack</button>`
      : `<button class="btn btn-gold" onclick="purchasePack('${p.id}')" style="width:100%;padding:8px;font-size:12px;">${priceDisplay}</button>`;
    return `
      <div style="border:1px solid var(--rule);border-radius:12px;overflow:hidden;background:var(--bg-canvas);">
        ...
        <div style="padding:10px;">
          ...
          <div style="font:11px/1 var(--font-body);color:var(--text-secondary);margin-bottom:4px;">by ${p.creator}</div>
...
window.renderOwnedPacks = function() {
  ...
  list.innerHTML = owned.map(p => `
    <div style="display:flex;align-items:center;gap:12px;padding:12px;border:1px solid var(--rule);border-radius:12px;margin-bottom:8px;">
      ...
      <button class="btn btn-outline" onclick="openPack('${p.id}')" style="padding:8px 16px;">Open</button>
    </div>
  `).join('');
};
