...
(async () => {
  ...
  try {
    for (const device of devices) {
      ...
      // Model a returning tester so successful authentication resumes at Home.
      // Authentication still gates the initial render because its session is tab-scoped.
      ...
      async function capture(id, label) {
        ...
        await page.screenshot({ path: path.join(SHOT_DIR, file), fullPage: true, animations: 'disabled' });
